            ...
            <div className="space-y-6">
              {news.length === 0 ? <p className="text-slate-500">Belum ada berita.</p> : 
                news.map(item => {
                  ...
                  return (
                    <div key={item.id} className="flex space-x-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100 items-start">
                      {item.imageUrl && (
                        <div className="w-24 h-24 shrink-0 bg-slate-100 rounded-lg overflow-hidden flex items-center justify-center">
                          {ytId ? (
                             <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${ytId}`} title={item.title} frameBorder="0" allowFullScreen></iframe>
                          ) : isVid ? (
                            <video src={item.imageUrl} className="w-full h-full object-cover" controls={false} muted autoPlay loop playsInline />
                             ...
                             <a href={item.imageUrl} target="_blank" rel="noreferrer" className="flex flex-col items-center">
                               <span className="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-1 rounded">PDF DOCUMENT</span>
                               <span className="text-[10px] text-slate-400 mt-1 uppercase tracking-tighter">Buka File</span>
                             </a>
                          ) : (
          ...
          <section>
            ...
            <div className="space-y-4">
              {events.length === 0 ? <p className="text-slate-500">Belum ada kegiatan.</p> : 
                events.map(event => {
                  const ytId = getYoutubeId(event.imageUrl || '');
                  const isVid = isVideoMedia(event.imageUrl || '');
                  const isPdf = event.imageUrl?.toLowerCase().includes('.pdf');
                  
                  return (
                    <div key={event.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex items-start space-x-4">
                       <div className="bg-emerald-100 text-emerald-800 p-2 rounded-lg text-center min-w-[60px] flex flex-col items-center justify-center">
                          <span className="block text-xl font-bold leading-none">{format(new Date(event.date), 'dd')}</span>
                          <span className="block text-[10px] uppercase font-bold mt-1 tracking-tighter">{format(new Date(event.date), 'MMM', { locale: id })}</span>
                       </div>
                       
                       {event.imageUrl && (
                         <div className="w-16 h-16 shrink-0 bg-slate-100 rounded-lg overflow-hidden border border-slate-50">
                           {ytId ? (
                              <div className="w-full h-full flex items-center justify-center bg-black"><span className="text-[8px] text-white">YOUTUBE</span></div>
                           ) : isVid ? (
                              <video src={event.imageUrl} className="w-full h-full object-cover" />
                           ) : isPdf ? (
                              <div className="w-full h-full flex items-center justify-center text-red-500 font-bold text-[10px]">PDF</div>
                           ) : (
                ...
                gallery.map(img => {
                  ...
                  return (
                    <div key={img.id} className="aspect-video relative group overflow-hidden rounded-xl bg-slate-200">
                      ...
                      ) : img.imageUrl && img.imageUrl.toLowerCase().includes('.pdf') ? (
                        <a href={img.imageUrl} target="_blank" rel="noreferrer" className="w-full h-full flex flex-col items-center justify-center bg-red-50 text-red-600">
                           <span className="font-black text-xl">PDF</span>
                           <span className="text-[10px] opacity-70">Klik untuk Buka</span>
                        </a>
                      ) : img.imageUrl ? (
