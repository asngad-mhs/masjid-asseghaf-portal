...
export function AdminKhotbah() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      alert('Gagal menambahkan khotbah.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8">
        <h3 className="font-semibold text-lg mb-4">Tambah Ringkasan Khotbah</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 text-right mt-2">
             <button disabled={loading} type="submit" className="bg-emerald-600 text-white px-6 py-2 rounded-lg hover:bg-emerald-700">Simpan Khotbah</button>
          </div>
        </form>
      </div>
